...
test("Electron exports exact icon centers and a resource-backed config", () => {
  ...
  expect(config.dmg.iconSize).toBe(128);
});
import { tauriConfig } from "../lib/dmgly/export/tauri";
test("Tauri uses the nested DMG config with its supported fields", () => {
  const d = createComposition(),
    r = tauriConfig(d),
...
test("native script is valid Bash and handles paths via quoted arguments", () => {
  const d = createComposition();
  d.app.name = "App ' & $(touch unsafe)";
  const r = nativeConfig(d);
